...
def test_trivia_newgame(mocker):
    ...
    mocker.patch('uuid.uuid4', return_value=uuid.UUID(
    ...
    app.MANAGEMENT.post_to_connection.assert_has_calls([
        mock.call(Data='{"action": "gamecreated", "gameId": "01234567012301230123012345678901"}',
                  ConnectionId='connection-123'),
...
def test_trivia_joingame(mocker):
    ...
    app.TABLE.query.return_value = {'Items': [
        {'connectionId': "connection-1", "playerName": "AliceBlue", "score": "10"}]}
...
def test_trivia_answer(mocker):
    ...
    app.TABLE.update_item.assert_called_with(
        Key={'gameId': '01234567012301230123012345678901',
             'connectionId': 'connection-answer-123'},
        AttributeUpdates={'lastQuestionId': {
            'Value': 'q-1234', 'Action': 'PUT'}, 'lastAnswer': {'Value': 'Answer', 'Action': 'PUT'}}
    )
...
def test_trivia_calculate_scores_correct(mocker):
    ...
    app.MANAGEMENT.post_to_connection.assert_has_calls([
        mock.call(
            Data='{"action": "playerlist", "players": [{"connectionId": "connection-1", "playerName": "AliceBlue", "score": 40, "currentPlayer": true}]}', ConnectionId='connection-1'),
